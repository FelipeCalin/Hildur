...
namespace Hildur {
	...
	Ref<Material> Material::Create(glm::vec3 ambient, glm::vec3 diffuse, glm::vec3 specular, Ref<Texture> diffuseTex, Ref<Texture> specularTex) {
		...
		return std::make_shared<Material>(ambient, diffuse, specular, diffuseTex, specularTex);
	...
	void Material::SendToShader(Ref<Shader> shader) {
		...
		std::dynamic_pointer_cast<Hildur::OpenGLShader>(shader)->UploadUniformInt("diffuseTex", m_DiffuseTex->GetTextureUnit());
		std::dynamic_pointer_cast<Hildur::OpenGLShader>(shader)->UploadUniformInt("specularTex", m_SpecularTex->GetTextureUnit());
